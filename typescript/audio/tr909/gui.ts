...
const installKnobs = (parentNode: ParentNode, preset: Preset): Terminable => {
    ...
    terminator.with(new Knob(HTML.query('[data-parameter=level]', hihatGroup), preset.hihat.level))
    terminator.with(new Knob(HTML.query('[data-parameter=cl-decay]', hihatGroup), preset.hihat.closedDecay))
    terminator.with(new Knob(HTML.query('[data-parameter=op-decay]', hihatGroup), preset.hihat.openedDecay))
